...
### 2026-02-XX — Prompt 5 (Reglas definitivas Nota / WhatsApp / Pase automático)
- ✅ **Nota MedSys**: introducción EXACTA "SE TRATA DE PACIENTE MASCULINO/FEMENINO DE XX AÑOS, NOMBRE, CON DIAGNÓSTICO DE __ POR LO QUE EL DÍA __ SE LE REALIZÓ __ CON HALLAZGOS DE __" (nunca separar cirugía de hallazgos). Sin clasificaciones (T4N3M1, EC IVA, ECOG, ROCKALL, ASA, CAPRINI, PADUA, LIGHT, "ANTECEDENTE DE"). Signos vitales INTERPRETADOS. Dolor con solo 3 fórmulas: "ADECUADO CONTROL ANALGÉSICO" / "MAL CONTROL ANALGÉSICO, REFIERE ENA EN X, POR LO QUE SE INDICÓ X" / "PARCIAL CONTROL...". Dieta con abreviaturas del servicio SIN EXPANDIR (BOOST/NPT/NET). "SIN REFERIR NÁUSEAS O VÓMITO". Labs INTERPRETADOS ("PERSISTE HIPOALBUMINEMIA (X)", "DISMINUCIÓN DE LEUCOCITOS DE X A X"). PLAN máximo 2 renglones separado por comas. Cierre "PRONÓSTICO RESERVADO A EVOLUCIÓN.".
- ✅ **WhatsApp**: SOLO primer nombre + (cama). NUNCA nombre completo, edad, diagnóstico, DPQX ni DEIH. Signos vitales interpretados. Labs solo si cambian conducta. Prohibido: "OJO", "llama la atención", "se sugiere", "se recomienda", "se informa que". Inicio "Buenos días Dr, pasé a ver a Juan (614), está con...". Cierre natural o UNA sola pregunta si hay decisión pendiente.
- ✅ **Pase automático (`/api/patients/{id}/generate/pase`)**: un único PÁRRAFO CONTINUO en mayúsculas siguiendo secuencia obligatoria: SIGNOS VITALES → OXÍGENO → DOLOR → DIETA → NÁUSEAS → DIURESIS → GASES → EVACUACIONES → DRENAJES → LABORATORIOS INTERPRETADOS → ESTUDIOS ("CON HALLAZGOS DE...") → CAMBIOS DE MEDICAMENTOS → INTERCONSULTAS → PROCEDIMIENTOS DE OTROS SERVICIOS → REPORTE HISTOPATOLÓGICO → POSTQUIRÚRGICO. Al final del párrafo, en línea aparte, PLAN corrido separado por comas (máximo 2 renglones). Sin bullets, sin listas, sin frases IA.
- ✅ **Test funcional**: `/app/tests/test_prompt5_final.py` valida las 3 generaciones con aserciones estrictas (introducción, ausencia de clasificaciones, fórmulas de dolor, ausencia de "SIN DATOS APORTADOS"/"DOLOR CONTROLADO", solo primer nombre en WhatsApp, ausencia de edad/dx/DPQX/DEIH en WhatsApp, ausencia de "OJO"/"se sugiere", PLAN en pase). **PASSED** en 4-7 s cada generación.
